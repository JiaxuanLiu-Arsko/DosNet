...
dos_raw_path = "/work/jxliu/e3nn/DOS_net/DosNet_pl/data/dos_raw"
...
for index in range(len(target_data)):
    ...
    all_scaling = np.max(np.sum(dos, axis=0))
    scaling = np.max(dos/all_scaling, axis=1)
    for i in range(0, len(ase_crystal)):
        dos[i,:] = (dos[i,:]/all_scaling)/scaling[i]
